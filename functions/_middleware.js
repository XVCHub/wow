...
export async function onRequest(context) {
  ...
  if (pathname === '/icon.png') {
    return await fetchFromGitHub('images', 'icon.png');
  }

  // check if it's a docs route
  if (pathname.startsWith('/docs/')) {
    const docPath = pathname.substring(1); // remove leading /
    
    // try to fetch index.html from that folder
    try {
      ...
      // if no index.html, continue to normal file handling
    }
  }
